...
static func find_and_connect_to_slots(node: Node2D, on_captured: Callable, on_released: Callable, on_denied: Callable) -> void:
	...
	for slot_area in node.get_tree().get_nodes_in_group("slots"):
		...
		if slot_component:
			...
			if on_denied:
				slot_component.denied_by_slot.connect(on_denied)
